...
const SOCKET_TOKEN = process.env.SOCKET_SERVER_TOKEN;
const ALLOWED_ORIGINS = (process.env.SOCKET_ALLOWED_ORIGINS || '').split(',').map(s => s.trim()).filter(Boolean);

if (!SOCKET_TOKEN) {
  console.warn('⚠️  SOCKET_SERVER_TOKEN no definido. Los endpoints /notify/* rechazarán todas las peticiones.');
}
...
// Validación de token para endpoints /notify/*
function requireSocketToken(req, res, next) {
 ...
 */
app.post('/notify/table-updated', requireSocketToken, (req, res) => {
 ...
 * Body: { room: 'tenant_1' }
 */
app.post('/notify/table-groups-updated', requireSocketToken, (req, res) => {
  const { room } = req.body;
 ...
 */
app.post('/notify/order-status-updated', requireSocketToken, (req, res) => {
